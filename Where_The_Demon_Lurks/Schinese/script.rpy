# TODO: Translation updated at 2021-11-07 21:50

translate Schinese strings:

    # game/script.rpy:41
    old "Vendrake{#speakingName}"
    new "Vendrake{#speakingName}"

    # game/script.rpy:42
    old "Nox{#speakingName}"
    new "Nox{#speakingName}"

    # game/script.rpy:43
    old "Fortis{#speakingName}"
    new "Fortis{#speakingName}"

    # game/script.rpy:44
    old "Amare{#speakingName}"
    new "Amare{#speakingName}"

    # game/script.rpy:45
    old "Unknown Person{#speakingName}"
    new "神秘人{#speakingName}"

    # game/script.rpy:47
    old "Son{#speakingName}"
    new "儿子{#speakingName}"

    # game/script.rpy:48
    old "Morris{#speakingName}"
    new "Morris{#speakingName}"

    # game/script.rpy:49
    old "Lucian{#speakingName}"
    new "Lucian{#speakingName}"

    # game/script.rpy:50
    old "Toast{#speakingName}"
    new "Toast{#speakingName}"

    # game/script.rpy:51
    old "Dog{#speakingName}"
    new "狗子{#speakingName}"

    # game/script.rpy:52
    old "Ghost{#speakingName}"
    new "幽灵{#speakingName}"

    # game/script.rpy:53
    old "Boar{#speakingName}"
    new "野猪{#speakingName}"

    # game/script.rpy:55
    old "King{#speakingName}"
    new "King{#speakingName}"

    # game/script.rpy:56
    old "Hood 1{#speakingName}"
    new "兜帽人 1{#speakingName}"

    # game/script.rpy:57
    old "Hood 2{#speakingName}"
    new "兜帽人 2{#speakingName}"

    # game/script.rpy:58
    old "Hood 3{#speakingName}"
    new "兜帽人 3{#speakingName}"

    # game/script.rpy:59
    old "Hood 4{#speakingName}"
    new "兜帽人 4{#speakingName}"

    # game/script.rpy:60
    old "Staff{#speakingName}"
    new "工作人员{#speakingName}"

    # game/script.rpy:114
    old "This visual novel may contain mature/suggestive themes,\nby proceeding forward you agree that you are of appropriate age from your \nrespective country to be exposed to these types of materials."
    new "该视觉小说可能包含成人/性暗示元素，\n在进行阅读之前，请确保您的年龄符合\n相关法律的规定。"

# TODO: Translation updated at 2022-12-08 00:20

translate Schinese strings:

    # game/script.rpy:52
    old "Gin{#speakingName}"
    new "Gin{#speakingName}"

    # game/script.rpy:53
    old "Gary{#speakingName}"
    new "Gary{#speakingName}"

    # game/script.rpy:54
    old "Kage{#speakingName}"
    new "Kage{#speakingName}"

    # game/script.rpy:55
    old "Male Nurse{#speakingName}"
    new "男护士{#speakingName}"

    # game/script.rpy:56
    old "Female Nurse{#speakingName}"
    new "女护士{#speakingName}"

    # game/script.rpy:68
    old "Waitress{#speakingName}"
    new "女服务员{#speakingName}"

    # game/script.rpy:69
    old "Imp Demon{#speakingName}"
    new "小恶魔{#speakingName}"

    # game/script.rpy:70
    old "Freelancer{#speakingName}"
    new "自业者{#speakingName}"

    # game/script.rpy:71
    old "Former Freelancer{#speakingName}"
    new "前自业者{#speakingName}"

    # game/script.rpy:72
    old "Eyeball{#speakingName}"
    new "眼球{#speakingName}"

    # game/script.rpy:73
    old "Ah Pa{#speakingName}"
    new "阿爸{#speakingName}"

    # game/script.rpy:74
    old "Ah Ma{#speakingName}"
    new "阿妈{#speakingName}"

    # game/script.rpy:75
    old "Wolf A{#speakingName}"
    new "狼 A{#speakingName}"

    # game/script.rpy:76
    old "Wolf B{#speakingName}"
    new "狼 B{#speakingName}"

    # game/script.rpy:77
    old "Tall Woman{#speakingName}"
    new "高个子的女人{#speakingName}"

    # game/script.rpy:78
    old "Short Man{#speakingName}"
    new "矮个子的男人{#speakingName}"

    # game/script.rpy:79
    old "Vendor{#speakingName}"
    new "小贩{#speakingName}"

    # game/script.rpy:80
    old "Person A{#speakingName}"
    new "路人 A{#speakingName}"

    # game/script.rpy:81
    old "Person B{#speakingName}"
    new "路人 B{#speakingName}"

    # game/script.rpy:82
    old "Mike{#speakingName}"
    new "Mike{#speakingName}"

# TODO: Translation updated at 2023-01-28 14:41

translate Schinese strings:

    # game/script.rpy:84
    old "Customer{#speakingName}"
    new "顾客{#speakingName}"

    # game/script.rpy:85
    old "Tall Freelancer{#speakingName}"
    new "高大的自业者{#speakingName}"

    # game/script.rpy:86
    old "Lady{#speakingName}"
    new "女人{#speakingName}"

    # game/script.rpy:87
    old "Dude{#speakingName}"
    new "男人{#speakingName}"

    # game/script.rpy:88
    old "Simulda{#speakingName}"
    new "Simulda{#speakingName}"

    # game/script.rpy:89
    old "Former Freelancer 2{#speakingName}"
    new "前自业者{#speakingName}"

# TODO: Translation updated at 2023-02-27 17:53

translate Schinese strings:

    # game/script.rpy:55
    old "Paw{#speakingName}"
    new "Paw{#speakingName}"

    # game/script.rpy:91
    old "Astaroth{#speakingName}"
    new "Astaroth{#speakingName}"

# TODO: Translation updated at 2023-06-27 17:17

# game/script.rpy:253
translate Schinese end_of_update_55566b14:

    # "{b}TO BE CONTINUED....{/b}"
    "{b}未完待续....{/b}"

# game/script.rpy:255
translate Schinese end_of_update_267ad187:

    # "Thanks for playing the latest update, if you want to let us know how we’re doing, leave a rating and comment on our itchio page. We want to hear from you!"
    "感谢您游玩最新的更新，如果您有任何意见或建议，欢迎前往我们的itchio页面进行留言和评分，我们衷心期待您的到来！"

# game/script.rpy:256
translate Schinese end_of_update_2b795aa9:

    # "Where the Demon Lurks is supported by those who sponsor us on Patreon! Consider supporting us so we can make the game bigger and better!"
    "《魔王藏身处》现已登陆Patreon平台！还请您多多支持，这样我们才能把游戏做得更好更强！"

# game/script.rpy:257
translate Schinese end_of_update_f1200ced:

    # "And if you want to be kept up-to-date on the development, feel free to follow our twitter or join our Discord!"
    "如果您想了解游戏的最新进展，记得随时关注我们的twitter或加入我们的Discord哦!"

translate Schinese strings:

    # game/script.rpy:71
    old "Qora{#speakingName}"
    new "Qora{#speakingName}"

    # game/script.rpy:94
    old "Uriel{#speakingName}"
    new "Uriel{#speakingName}"

    # game/script.rpy:95
    old "Sariel{#speakingName}"
    new "Sariel{#speakingName}"

