# TODO: Translation updated at 2023-02-15 21:54

translate Tchinese strings:

    # game/screens.rpy:580
    old "About"
    new "關於"

    # game/screens.rpy:587
    old "{font=font/DenseLetters-nRgDO.otf}Version{/font} [config.version!t]\n"
    new "{font=font/DenseLetters-nRgDO.otf}Version{/font} [config.version!t]\n"

    # game/screens.rpy:593
    old "Made with {a=https://www.renpy.org/}Ren'Py{/a} [renpy.version_only].\n\n[renpy.license!t]"
    new "使用{a=https://www.renpy.org/}Ren'Py{/a} [renpy.version_only]制作。\n\n[renpy.license!t]"

    # game/screens.rpy:621
    old "Save"
    new "保存"

    # game/screens.rpy:628
    old "Load"
    new "載入"

    # game/screens.rpy:633
    old "Page {}"
    new "第{}頁"

    # game/screens.rpy:633
    old "Automatic saves"
    new "自動保存"

    # game/screens.rpy:633
    old "Quick saves"
    new "快速保存"

    # game/screens.rpy:675
    old "{#file_time}%A, %B %d %Y, %H:%M"
    new "{#file_time}%A, %B %d %Y, %H:%M"

    # game/screens.rpy:675
    old "empty slot"
    new "空存檔位"

    # game/screens.rpy:692
    old "<"
    new "<"

    # game/screens.rpy:695
    old "{#auto_page}A"
    new "{#auto_page}A"

    # game/screens.rpy:698
    old "{#quick_page}Q"
    new "{#quick_page}Q"

    # game/screens.rpy:704
    old ">"
    new ">"

    # game/screens.rpy:749
    old "Preferences"
    new "設定"

    # game/screens.rpy:760
    old "Display"
    new "顯示"

    # game/screens.rpy:761
    old "Window"
    new "視窗化"

    # game/screens.rpy:762
    old "Fullscreen"
    new "全螢幕"

    # game/screens.rpy:766
    old "Rollback Side"
    new "點擊螢幕回退"

    # game/screens.rpy:767
    old "Disable"
    new "關閉"

    # game/screens.rpy:768
    old "Left"
    new "螢幕左側"

    # game/screens.rpy:769
    old "Right"
    new "螢幕右側"

    # game/screens.rpy:773
    old "Skip"
    new "快進"

    # game/screens.rpy:774
    old "Unseen Text"
    new "未讀文本"

    # game/screens.rpy:775
    old "After Choices"
    new "選擇後繼續快進"

    # game/screens.rpy:776
    old "Transitions"
    new "忽略轉場"

    # game/screens.rpy:780
    old "Language"
    new "語言"

    # game/screens.rpy:781
    old "{font=font/HYYuanLongHei-90W.ttf}简体中文{/font}"
    new "{font=font/HYYuanLongHei-90W.ttf}简体中文{/font}"

    # game/screens.rpy:784
    old "{font=font/GenSenRounded-H.ttc}繁體中文{/font}"
    new "{font=font/GenSenRounded-H.ttc}繁體中文{/font}"

    # game/screens.rpy:794
    old "Font"
    new "字體"

    # game/screens.rpy:796
    old "{font=font/DenseLetters-nRgDO.otf}English"
    new "{font=font/DenseLetters-nRgDO.otf}English"

    # game/screens.rpy:799
    old "{font=font/OpenDyslexicReg.otf}OpenDyslexic"
    new "{font=font/OpenDyslexicReg.otf}OpenDyslexic"

    # game/screens.rpy:816
    old "Discord Integration"
    new "Discord整合"

    # game/screens.rpy:818
    old "Off"
    new "Off"

    # game/screens.rpy:819
    old "On"
    new "On"

    # game/screens.rpy:832
    old "Text Speed"
    new "文字顯示速度"

    # game/screens.rpy:836
    old "Auto Forward Time"
    new "自動播放間隔"

    # game/screens.rpy:844
    old "Music Volume"
    new "音樂音量"

    # game/screens.rpy:851
    old "Sound Volume"
    new "音效音量"

    # game/screens.rpy:857
    old "Test"
    new "測試"

    # game/screens.rpy:862
    old "Mute All"
    new "全部靜音"

    # game/screens.rpy:955
    old "History"
    new "歷史"

    # game/screens.rpy:985
    old "The dialogue history is empty."
    new "無對話歷史紀錄。"

    # game/screens.rpy:1107
    old "Skipping"
    new "快進中"

    # game/screens.rpy:1327
    old "Back"
    new "回退"

    # game/screens.rpy:1329
    old "Auto"
    new "自動"

    # game/screens.rpy:1330
    old "Menu"
    new "主選單"

